...
Future<void> downloadAttachment(String url, int index, String extension) async {
  ...
  if (response.statusCode == 200) {
    ...
    await saveAndOpenFile(bytes, '${MyStrings.appName}_${DateTime.now()}.$extension', extension);
  } else {
    try {
...
Future<void> saveAndOpenFile(Uint8List bytes, String fileName, String extension) async {
  final directory = await getExternalStorageDirectory();
  if (directory != null) {
    ...
    if (extension == 'pdf' || extension == 'doc' || extension == 'docx') {
      ...
      CustomSnackBar.error(errorList: [MyStrings.unsupportedFileType]);
    }
  } else {
    CustomSnackBar.error(errorList: [MyStrings.unableToAccessStorage]);
  }
}
